...
const reviewLines = computed<ReviewLine[]>(() => [
  { label: t('transaction.cashPaidCustomer'), value: payoutTotal.value, signed: 'debit' },
  ...
  { label: t('transaction.floatAfterPayout'), value: floatBalance.value - payoutTotal.value },
])
  ...
  <TellerLayout :float="float">
    <template v-if="completed">
      <header class="mb-6 text-center">
        <h1 class="font-display text-2xl font-semibold tracking-tight">{{ t('transaction.cashOut') }} ပြီးပါပြီ</h1>
        <p class="mt-1 text-sm text-ink-700/70">ဖောက်သည်ရှေ့မှာ ငွေစက္ကူများကို ရေတွက်ပေးပါ။</p>
      </header>
      <ReceiptSlip :txn="completed" next-href="/teller/cash-out" :next-label="t('transaction.newCashOut')" />
    </template>

    <template v-else>
      <header class="mb-5">
        <h1 class="font-display text-2xl font-semibold tracking-tight">{{ t('transaction.cashOut') }}</h1>
      ...
      <div class="grid gap-6 lg:grid-cols-[1fr_20rem]">
        <div class="space-y-5">
          <section class="rounded-counter border border-paper-edge bg-white p-5">
            <div class="grid gap-5 sm:grid-cols-2">
              <div class="sm:col-span-2">
                <AccountPicker v-model="accountId" :accounts="accounts" :label="t('transaction.accountCredit')" />
              </div>
              <div class="sm:col-span-2">
                <AmountField v-model="amount" :label="t('transaction.cashOutAmount')" />
              </div>
              <div>
                <label class="field-label" for="name">ဖောက်သည်အမည်</label>
              ...
              <div class="sm:col-span-2">
                ...
                <p class="mt-1 text-xs text-ink-700/55">
                  သတ်မှတ်ထားသော ဝန်ဆောင်ခအဆင့်နှင့် MMK အကြွေစည်းမျဉ်းအတိုင်း တွက်ထားပြီး ဒီနေရာမှာ ပြင်လို့မရပါ။
                </p>
              </div>
            </div>
          </section>
          ...
          <p v-if="shortFloat" class="rounded-counter border border-debit/30 bg-debit/5 px-4 py-2.5 text-sm text-debit">
            ကိုယ်ပိုင်ငွေခွဲမှာ <MoneyText :value="floatBalance" class="font-semibold" /> ရှိပါတယ်။ ပမာဏလျှော့ပါ သို့မဟုတ် Cashier ထံမှ ငွေထပ်တောင်းပါ။
          </p>
        ...
        <aside class="h-fit rounded-counter border border-ink-800 bg-ink-900 p-5 text-ink-100 lg:sticky lg:top-24">
          ...
          <dl class="mt-4 space-y-3 text-sm">
            <div class="flex justify-between"><dt class="text-ink-300">{{ t('transaction.cashPaidCustomer') }}</dt><dd><MoneyText :value="payoutTotal" /></dd></div>
            ...
            <div class="flex justify-between"><dt class="text-ink-300">{{ t('transaction.floatAfterPayout') }}</dt>
          ...
          <p class="mt-2.5 text-center text-[11px] leading-relaxed text-ink-300">
            အတည်ပြုပြီးသည်နှင့် ငွေထုတ်စာရင်း ပြီးမြောက်ပါမယ်။ Cashier အတည်ပြုရန် မလိုပါ။
          </p>
