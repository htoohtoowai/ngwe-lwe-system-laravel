...
const reviewLines = computed<ReviewLine[]>(() => [
  { label: 'Cash movement counted', value: payoutTotal.value, signed: 'debit' },
  { label: 'Fee from commission tier', value: feeNum.value },
  { label: 'Source account debited', value: amount.value || 0, signed: 'debit' },
  { label: 'Destination account credited', value: amount.value || 0, signed: 'credit', emphasize: true },
  { label: 'Your float after transfer', value: floatBalance.value - payoutTotal.value },
])
...
function submit() {
  ...
  router.post('/employee/transactions/transfer', {
    _token: csrfToken(),
  ...
  <EmployeeLayout :float="float">
    <template v-if="completed">
      <header class="mb-6 text-center">
        <h1 class="font-display text-2xl font-semibold tracking-tight">Transfer completed</h1>
        <p class="mt-1 text-sm text-ink-700/70">Show this reference after both accounts are checked.</p>
      </header>
      <ReceiptSlip :txn="completed" next-href="/employee/transfer" next-label="Next Transfer" />
    </template>

    <template v-else>
      <header class="mb-5">
        <h1 class="font-display text-2xl font-semibold tracking-tight">Transfer</h1>
        <p class="mt-1 text-sm text-ink-700/70">
          Move value between accounts. Count the cash movement note by note before recording.
        </p>
      </header>
        ...
        <div class="space-y-5">
          <section class="rounded-counter border border-paper-edge bg-white p-5">
            <div class="grid gap-5">
              <AccountPicker v-model="fromAccountId" :accounts="accounts" :must-cover="amount" label="Source account" />
              <AccountPicker v-model="toAccountId" :accounts="accounts" label="Destination account" />
              <AmountField v-model="amount" label="Transfer amount" />
              <div>
          ...
          <DenominationDrawer
            ...
            :target="amount || 0"
            :stock="floatStock"
            label="Notes counted for transfer"
          />
        </div>
        ...
        <aside class="h-fit rounded-counter border border-ink-800 bg-ink-900 p-5 text-ink-100 lg:sticky lg:top-24">
          ...
          <dl class="mt-4 space-y-3 text-sm">
            ...
            <div class="flex justify-between border-t border-ink-800 pt-3">
              <dt class="font-semibold">Destination credited</dt>
            ...
            <div class="flex justify-between"><dt class="text-ink-300">Float after transfer</dt><dd><MoneyText :value="floatBalance - payoutTotal" /></dd></div>
          </dl>
      ...
      <ReviewSheet
        ...
        consequence="On confirm, the source account is debited, the destination account is credited, and these exact notes are deducted from your float."
        @confirm="submit"
