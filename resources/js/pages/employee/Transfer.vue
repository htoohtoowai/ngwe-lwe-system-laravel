...
watch([amount, fromAccountId], ([value, selectedAccount]) => {
  if (value > 0 && selectedAccount) {
    router.reload({ only: ['fee'], data: { amount: value, from_account_id: selectedAccount }, headers: authHeaders() })
  }
})
...
async function submit() {
  ...
  try {
    await apiRequest('/api/transactions/transfer', {
      method: 'POST',
    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_20rem]">
      <div class="space-y-5">
        <section class="rounded-counter border border-paper-edge bg-white p-5">
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              <label class="field-label" for="from-account">Source account</label>
              <select id="from-account" v-model="fromAccountId" class="field-input mt-1.5">
                <option :value="null" disabled>Choose source</option>
                <option v-for="a in accounts" :key="a.id" :value="a.id">{{ a.company }} - {{ a.name }}</option>
              </select>
            </div>
            <div>
              <label class="field-label" for="to-account">Destination account</label>
              <select id="to-account" v-model="toAccountId" class="field-input mt-1.5">
                <option :value="null" disabled>Choose destination</option>
                <option v-for="a in accounts" :key="a.id" :value="a.id">{{ a.company }} - {{ a.name }}</option>
              </select>
            </div>
            <div>
              <label class="field-label" for="amount">Transfer amount</label>
      ...
      <aside class="h-fit rounded-counter border border-ink-800 bg-ink-900 p-5 text-ink-100 lg:sticky lg:top-24">
        ...
        <dl class="mt-4 space-y-3 text-sm">
          ...
          <div class="flex justify-between">
            <dt class="text-ink-300">Float after transfer</dt>
        ...
        <button type="button" :disabled="!ready || submitting" @click="submit"
          ...
          {{ submitting ? 'Recording...' : 'Complete transfer' }}
        </button>
