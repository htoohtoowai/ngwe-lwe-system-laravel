...
const filtered = computed(() => {
    ...
    return rows.value.filter((row) =>
        query === '' ||
        [row.txn_type, row.note, row.performed_by_name, row.denomination, row.quantity]
    ...
    <BankLayout :role="role" :notification-count="notificationCount">
        ...
        <section class="mt-5 rounded-xl border border-line bg-card p-5 shadow-sm">
            <div class="flex gap-2">
                <input v-model="search" type="search" class="bank-input max-w-md" placeholder="Search type, note, user" />
                <button class="bank-button bank-button-secondary" :disabled="loading" @click="load">Refresh</button>
            </div>
            <div class="mt-4 overflow-auto rounded-lg border border-line">
                <table class="w-full min-w-[760px] text-left text-sm">
                    <thead class="bg-mist text-xs text-slate uppercase"><tr><th class="px-4 py-3">Time</th><th class="px-4 py-3">Type</th><th class="px-4 py-3">Note</th><th class="px-4 py-3 text-right">Qty</th><th class="px-4 py-3">By</th></tr></thead>
                    <tbody class="divide-y divide-line">
                        <tr v-for="log in paginated" :key="log.id"><td class="px-4 py-3 text-xs text-slate">{{ dateTime(log.created_at) }}</td><td class="px-4 py-3 font-bold">{{ log.txn_type }}</td><td class="px-4 py-3 text-slate">{{ log.note ?? '-' }}</td><td class="money px-4 py-3 text-right">{{ money(log.denomination) }} × {{ log.quantity }}</td><td class="px-4 py-3 text-slate">{{ log.performed_by_name ?? '-' }}</td></tr>
                        <tr v-if="!paginated.length"><td colspan="5" class="px-4 py-8 text-center text-slate">No vault logs found.</td></tr>
                    </tbody>
                </table>
            </div>
