...
type CalculationTypeSelection = CalculationType | '';
...
const firstCompany = 0;
const firstAgentCompany = 0;
const secondCompany = 0;
const firstFeature = '';
...
const filteredAgentTiers = computed(() => filterRows(props.agentCommissionTiers, (tier) => [
    tier.company_name,
    agentCommissionForLabel(tier.feature),
    tier.amount_from,
    tier.amount_to,
]));
    ...
    <BankLayout :role="role" :announcement="announcement" :notification-count="notificationCount">
        ...
        <form v-if="isEditor && activeKind === 'provider'" class="bank-form-shell mt-4 max-w-5xl" @submit.prevent="submit('provider')">
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <label><span class="bank-label bank-required">Provider</span><select v-model.number="providerForm.company_id" class="bank-input min-h-13" required><option :value="0" disabled>Select provider</option><option v-for="company in companies" :key="company.id" :value="company.id" :disabled="!company.is_active">{{ company.name }}{{ company.is_active ? '' : ' (Inactive)' }}</option></select></label>
                <label><span class="bank-label bank-required">Fee For</span><select v-model="providerForm.feature" class="bank-input min-h-13" required><option value="" disabled>Select fee category</option><option v-for="feature in features" :key="feature.value" :value="feature.value">{{ feature.label }}</option></select></label>
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-2">
                <fieldset>
                    ...
                    <div class="grid grid-cols-[8rem_minmax(0,1fr)_auto] items-center gap-2">
                        <select v-model="providerForm.fee_type" class="bank-input min-h-13" required><option value="" disabled>Select calculation type</option><option v-for="type in calculationTypes" :key="type.value" :value="type.value">{{ type.value }}</option></select>
                ...
                <fieldset>
                    ...
                    <div class="grid grid-cols-[8rem_minmax(0,1fr)_auto] items-center gap-2">
                        <select v-model="providerForm.additional_fee_type" class="bank-input min-h-13" required><option value="" disabled>Select calculation type</option><option v-for="type in calculationTypes" :key="type.value" :value="type.value">{{ type.value }}</option></select>
        ...
        <form v-else-if="isEditor && activeKind === 'agent'" class="bank-form-shell mt-4 max-w-5xl" @submit.prevent="submit('agent')">
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <label><span class="bank-label bank-required">Provider</span><select v-model.number="agentForm.company_id" class="bank-input min-h-13" required><option :value="0" disabled>Select provider</option><option v-for="company in agentCompanies" :key="company.id" :value="company.id" :disabled="!company.is_active">{{ company.name }}{{ company.is_active ? '' : ' (Inactive)' }}</option></select></label>
                <label><span class="bank-label bank-required">Commission For</span><select v-model="agentForm.feature" class="bank-input min-h-13" required><option value="" disabled>Select commission category</option><option v-for="feature in features" :key="feature.value" :value="feature.value">{{ agentCommissionForLabel(feature.value) }}</option></select></label>
            ...
            <fieldset class="mt-4">
                ...
                <div class="grid gap-3 md:grid-cols-[10rem_minmax(0,1fr)_minmax(0,1fr)]">
                    <label><span class="bank-label bank-required">Calculation Type</span><select v-model="agentForm.commission_type" class="bank-input min-h-13" required><option value="" disabled>Select calculation type</option><option v-for="type in calculationTypes" :key="type.value" :value="type.value">{{ type.value }}</option></select></label>
                    <label><span class="bank-label bank-required">Out Commission</span><div class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2"><input v-model.number="agentForm.out_commission_value" type="number" min="0" step="0.0001" class="bank-input money min-h-13" required /><span class="text-xs font-black text-slate">{{ unit(agentForm.commission_type) }}</span></div></label>
                    <label><span class="bank-label bank-required">In Commission</span><div class="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2"><input v-model.number="agentForm.in_commission_value" type="number" min="0" step="0.0001" class="bank-input money min-h-13" required /><span class="text-xs font-black text-slate">{{ unit(agentForm.commission_type) }}</span></div></label>
                </div>
            </fieldset>
            ...
            <div class="mt-5 grid gap-3 border-t border-line pt-4 md:grid-cols-[minmax(12rem,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto] md:items-end">
                <label><span class="bank-label">Preview amount</span><input v-model.number="sampleAmount" type="number" min="1" step="1" class="bank-input money min-h-13" /></label>
                <div class="flex min-h-13 items-center justify-between rounded-field bg-mist px-4 py-2"><span class="text-xs font-bold text-slate">Out Commission</span><strong class="money text-lg text-balance">+{{ money(agentOutPreview) }} MMK</strong></div>
                <div class="flex min-h-13 items-center justify-between rounded-field bg-mist px-4 py-2"><span class="text-xs font-bold text-slate">In Commission</span><strong class="money text-lg text-balance">+{{ money(agentInPreview) }} MMK</strong></div>
        ...
        <form v-else-if="isEditor && activeKind === 'transfer'" class="bank-form-shell mt-4 max-w-5xl" @submit.prevent="submit('transfer')">
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <label><span class="bank-label bank-required">From provider</span><select v-model.number="transferForm.company_from_id" class="bank-input min-h-13" required><option :value="0" disabled>Select source provider</option><option v-for="company in companies" :key="company.id" :value="company.id" :disabled="!company.is_active">{{ company.name }}</option></select></label>
                <label><span class="bank-label bank-required">To provider</span><select v-model.number="transferForm.company_to_id" class="bank-input min-h-13" required><option :value="0" disabled>Select destination provider</option><option v-for="company in companies" :key="company.id" :value="company.id" :disabled="!company.is_active || company.id === transferForm.company_from_id">{{ company.name }}</option></select></label>
            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <h2 class="mt-1 text-xl font-black">
                        ...
                        <template v-else-if="activeKind === 'agent' && currentAgent">{{ currentAgent.company_name }} / {{ agentCommissionForLabel(currentAgent.feature) }}</template>
        ...
        <section v-else class="mt-4">
            <div class="grid gap-2 md:grid-cols-[minmax(0,1fr)_12rem]">
                <input v-model="search" type="search" class="bank-input min-h-12" placeholder="Search provider, type, route or amount" />
