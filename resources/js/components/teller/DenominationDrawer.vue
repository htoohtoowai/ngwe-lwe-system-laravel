...
<template>
  <section class="rounded-counter border border-paper-edge bg-white" role="group" :aria-label="label ?? t('component.notesCounted')">
    <header class="flex items-center justify-between border-b border-paper-edge px-4 py-2.5">
      <div>
        <h3 class="field-label">{{ label ?? t('component.notesCounted') }}</h3>
        <p class="mt-0.5 text-xs text-ink-700/60">{{ noteCount }} {{ t('teller.notes') }}</p>
      </div>
    ...
    <ul class="divide-y divide-paper-edge">
      <li v-for="n in notes" :key="n"
        ...
        <span v-if="stock" class="hidden w-24 shrink-0 text-[11px] text-ink-700/55 sm:block">
          {{ (stock[n] ?? 0).toLocaleString() }} {{ t('component.onHand') }}
        </span>
        <span v-else-if="expected" class="hidden w-24 shrink-0 text-[11px] text-ink-700/55 sm:block">
          {{ (expected[n] ?? 0).toLocaleString() }} {{ t('component.issued') }}
        </span>
        ...
        <div class="ml-auto flex items-center gap-1" @click.stop>
          <button type="button" :disabled="readonly || qty(n) === 0" @click="set(n, qty(n) - 1)"
                  ...
                  :aria-label="`လျော့ရန် ${n.toLocaleString()}`">-</button>
          <input
          ...
          <button type="button" :disabled="readonly || qty(n) >= (stock?.[n] ?? Infinity)" @click="set(n, qty(n) + 1)"
                  class="bank-button bank-button-secondary grid size-8 min-h-8 place-items-center rounded-counter p-0 disabled:opacity-30"
                  :aria-label="`တိုးရန် ${n.toLocaleString()}`">+</button>
        </div>
    ...
    >
      ...
      <template v-if="target !== null">
        ...
        <div class="text-right">
          <p class="field-label">{{ balanced ? t('component.balanced') : difference > 0 ? t('component.overBy') : t('component.shortBy') }}</p>
