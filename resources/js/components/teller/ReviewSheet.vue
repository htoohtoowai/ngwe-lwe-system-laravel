...
<template>
  <div v-if="open" class="fixed inset-0 z-50 grid place-items-end bg-ink-950/60 p-0 sm:place-items-center sm:p-4"
    ...
    <div class="w-full rounded-t-2xl border border-ink-800 bg-white shadow-2xl sm:max-w-md sm:rounded-counter">
      ...
      <dl class="divide-y divide-dashed divide-paper-edge px-5">
        <div v-for="line in lines" :key="line.label"
          ...
          <dt class="text-sm" :class="line.emphasize ? 'text-ink-900' : 'text-ink-700'">{{ line.label }}</dt>
          <dd><MoneyText :value="line.value" :signed="line.signed ?? null"
                         :class="line.emphasize ? 'text-lg' : 'text-sm'" /></dd>
        </div>
      </dl>

      <p class="border-t border-paper-edge bg-paper px-5 py-3 text-xs leading-relaxed text-ink-700/80">
        {{ consequence }}
      </p>
