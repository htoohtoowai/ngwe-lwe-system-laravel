...
<template>
    <div>
        ...
        >
            ...
            <p v-else class="flex-1 text-sm text-slate">
                {{ t('component.chooseAccount') }}
            </p>
        ...
        <Teleport to="body">
            <div
                v-if="open"
                class="fixed inset-0 z-50 grid place-items-end bg-ink/40 p-0 sm:place-items-center sm:p-4"
                role="presentation"
                tabindex="-1"
                @keydown.esc="open = false"
            >
                <div
                    class="max-h-[80vh] w-full overflow-hidden rounded-t-2xl bg-card shadow-2xl sm:max-w-md sm:rounded-2xl"
                    role="dialog"
                    aria-modal="true"
                    :aria-label="label"
                >
                    ...
                    >
                        ...
                        <button
                            type="button"
                            class="bank-button grid size-9 place-items-center rounded-full p-0 hover:bg-mist"
                    ...
                    <div class="border-b border-line p-3">
                        <label
                            class="sr-only"
                            :for="`account-search-${label}`"
                            >{{ t('component.searchAccount') }}</label
                        >
                        <input
                            :id="`account-search-${label}`"
                            v-model="query"
                            type="search"
                            autocomplete="off"
                    ...
                    <ul
                        class="max-h-[50vh] divide-y divide-line overflow-y-auto"
                    >
                        <li v-for="a in list" :key="a.id">
                            <button
                                type="button"
                                :aria-pressed="a.id === modelValue"
                                @click="choose(a.id)"
                        ...
                        <li
                            v-if="!list.length"
                            class="px-5 py-10 text-center text-sm text-slate"
                        >
                            {{ t('common.noResults') }}
                        </li>
                    </ul>
